...
import { JULY_EVENTS } from '../../lib/catarsis.data';
import type { EventCard } from '../../types/catarsis.types';
...
function EventCardItem({ event, index, reduced }: { event: EventCard; index: number; reduced: boolean | null }) {
  return (
    <motion.article
      ...
      style={{
        background: `linear-gradient(160deg, ${event.colorBg} 0%, rgba(8,5,5,0.97) 45%, #080505 100%)`,
    ...
    >
      ...
      <div className="relative z-10 p-5 flex flex-col gap-3 flex-1">
        ...
        <div className="flex flex-col gap-0.5">
          ...
          <p className="text-white/40 text-xs">{event.time} · Parque Infantil</p>
        </div>
...
export function EventSection() {
  ...
  return (
    <section id="eventos" className="relative py-16 md:py-24 overflow-hidden" style={{ background: '#050505' }}>
      <div
        className="absolute inset-0 pointer-events-none"
        style={{
          background:
            'radial-gradient(ellipse 60% 50% at 50% 50%, rgba(229,9,20,0.05) 0%, transparent 70%)',
        }}
      />
      ...
      <div className="max-w-6xl mx-auto px-5 md:px-10">
        ...
        >
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1 h-px" style={{ background: 'rgba(229,9,20,0.15)' }} />
            <span className="text-white/25 text-xs">Pasto, Colombia</span>
          </div>
        </motion.div>

        {/* 4-card grid: 2 cols on mobile, 4 on desktop */}
